# WhyWorldwise SVG Icons Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace 4 emoji icons in `WhyWorldwise.tsx` with inline SVG icons styled in gold to match the site's luxury navy/gold brand.

**Architecture:** Single file edit — change the `points` array icon field from a string emoji to a JSX `ReactNode` containing an inline SVG. Wrap each SVG in a `w-12 h-12 rounded-full bg-gold/10` container. No new dependencies.

**Tech Stack:** Next.js 14, TypeScript, Tailwind CSS (navy/gold palette)

---

### Task 1: Rewrite WhyWorldwise.tsx with SVG icons

**Files:**
- Modify: `worldwise/components/WhyWorldwise.tsx`

- [ ] **Step 1: Replace the file contents**

Open `worldwise/components/WhyWorldwise.tsx` and replace with:
...
```bash
cd worldwise && export PATH="/Users/dzhambulat/.nvm/versions/node/v24.15.0/bin:$PATH" && npm run build
```
...
```bash
git add worldwise/components/WhyWorldwise.tsx worldwise/docs/superpowers/specs/2026-05-10-whyworldwise-icons-design.md worldwise/docs/superpowers/plans/2026-05-10-whyworldwise-icons.md
git commit -m "Replace emoji icons with inline SVG in WhyWorldwise section"
```
