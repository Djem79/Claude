...
> **✅ ПРИМЕНЕНО 16.07.2026, все 42/42 видео** — через YouTube Data API (`scripts/youtube.mjs apply`, волны 1→2→3, каждая правка верифицирована readback'ом + публичная спот-проверка yt-dlp). Браузерный путь (chrome-devtools) не сработал: Google блокирует вход в аккаунт в автоматизированном Chrome («браузер небезопасен») — отсюда API-скрипт. Оригиналы описаний сохранены в `~/Documents/worldwise-thumbnails/yt-{videos,shorts}-full.jsonl` (откат возможен). Разделы ниже — исходный аудит и материалы.
