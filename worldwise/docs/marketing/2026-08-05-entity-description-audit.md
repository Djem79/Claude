...
`orgJsonLd` несёт `aggregateRating` 5.0 из 4 отзывов, собранных на нашем же сайте. Отзывы о
себе, размещённые у себя, не дают права на rich-результат и относятся к «self-serving reviews»
в политике структурированных данных Google. Пользы ноль, риск ненулевой. Варианты: убрать
совсем либо заменить на агрегат с внешней площадки (Google Business / PF), где отзывы
проверяемы.
