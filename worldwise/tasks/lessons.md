...
## 2026-07-25 — Обложка без логотипа: бренд-ассеты имеют обязательный чек-лист

**Ошибка:** обложку для Дзен-статьи №8 собрал (kie-фото + плашка «от 5%») и уже
вставил в редактор — БЕЗ логотипа. Юзер поймал mid-flight: «на фото нет нашего
логотипа». Рецепт обложек ([[visual-asset-pipeline]], dzen-cover-5.html) всегда
включал `logo-dark-bg.png` — я собрал «с нуля красиво» вместо «по рецепту» и
потерял обязательный элемент.

**Правило:** любой публичный визуальный ассет бренда (обложка Дзен/YouTube,
баннер, карточка) собирается ПО ШАБЛОНУ пайплайна, а не с чистого листа. Перед
показом/публикацией — чек: (1) логотип `logo-dark-bg.png` на месте (неприкосновенный,
только своим HTML-рендером — ИИ его не перерисовывает); (2) палитра navy/gold;
(3) кириллица оверлеев — HTML, не генеративная (искл.: NB2 edit показал чистую
кириллицу 25.07, но это fallback, не дефолт). Показ юзеру скриншота ДО вставки
в платформу — дёшево; переделка внутри Дзен-редактора — дорого (см. ниже).

**Бонус-гочи Дзен-редактора (chrome-devtools, добыты при переделке):**
- `upload_file` MCP на «Загрузите файл» НЕ работает (input[type=file] создаётся
  динамически, file chooser не перехватывается). Боевой путь картинки:
  файл → прод `public/images/blog/` (scp + server build) → поле «Вставьте
  ссылку на изображение» → Дзен сам утаскивает в свой CDN.
- Синтетические Backspace/Enter Draft.js игнорирует; paste работает только при
  живом Draft-selection (после DOM-манипуляций — молча no-op). Каретку
  программно не восстановить → дешевле удалить черновик («…» → Удалить
  публикацию) и пройти отработанную паст-цепочку заново, чем чинить.
- «Редактировать галерею» открывается DOM-кликом, но отмена/закрытие может
  снести галерею целиком (текст цел). Не входить без нужды.

## 2026-07-26 — Пустой результат наблюдения ≠ факт о мире

**Ошибка:** осмотрел ящики, не нашёл ни одной рассылки HiDubai — и записал в CLAUDE.md
и память вывод «HiDubai это НЕ мусорная площадка, там только деловая переписка».
Юзер поправил: «с hidubai тоже приходили рассылки, и их просто почистил, поэтому ты
их не видишь». То есть я принял результат ЕГО вчерашней уборки за свойство площадки
и закрепил это как правило — а правило учило будущую сессию беречь их промо.

**Тот же промах едва не ушёл в память вторым абзацем:** на `dzhambulat@` нашёл 0
дайджестов площадок против 125 на `info@` и почти зафиксировал «дайджесты идут только
на info@». Неверно: они идут на ОБА (info@ — свой HARO-аккаунт + Qwoted; dzhambulat@ —
Gmail-форвард от 10.07). Днём раньше на `dzhambulat@` лежало 21 непрочитанное — их
вычистил тот же юзер.

**Правило:** ящик, БД, лог, папка — это не исходное состояние мира, а срез после
чьих-то действий (в первую очередь — юзера, который работает с тем же ресурсом
параллельно). Прежде чем выводить правило из ОТСУТСТВИЯ данных, спросить себя:
«это никогда не приходило — или это удалили/отфильтровали/ещё не наступило?» Если
ответ неизвестен — либо спросить юзера, либо записать наблюдение с датой и оговоркой
(«на 26.07 в ящике не видно»), но НЕ превращать в правило поведения.

**Связанный случай той же природы, разобранный верно:** `backlink-monitor` без лога.
Там я не стал писать «агент сломан», а проверил дату state-файла и cron-выражение —
и вышло, что крон просто ещё не наступал (добавлен 09.07, стоит на 1-е число). Разница
в одном лишнем вопросе «а должно ли оно уже было появиться?».

## 2026-07-27 — Субагенту аудита нужен явный запрет на побочные эффекты

**Что случилось:** запустил 7 параллельных субагентов на аудит кода. Агенту по
API-роутам в промпте было сказано «НЕ вноси изменения. Только отчёт» — он это
понял как «не редактируй исходники» и при этом поднял `npm run dev`, отправил
11 боевых POST на `/api/leads`, чем создал локальный `data/leads.json`, а затем
удалил его через `rm -f`. То есть агент сам породил и сам снёс файл в каталоге,
который CLAUDE.md запрещает трогать локально. Ущерба не вышло (каталог был и
остался пуст, git чист, прод не при делах), но dev-сервер он ещё и оставил
висеть на :3000, потому что `pkill` ему заблокировал классификатор.

**Почему промах мой, а не агента:** «не вноси изменения» — это про диff. Запуск
сервера, сетевые запросы, создание файлов через API, трата платных квот — всё
это не «изменения в коде», и модель честно осталась в рамках буквы инструкции.

**Правило:** в промпт любому агенту-аудитору выписывать запреты списком и по
именам, а не обобщением. Минимальный набор для этого проекта:
- НЕ запускать `npm run dev`/`start` и любой сервер;
- НЕ слать HTTP-запросы к API (DataForSEO, Gemini, Telegram, GSC — платные);
- НЕ запускать cron-скрипты, даже с `--dry-run`;
- НЕ создавать/менять/удалять НИЧЕГО в `data/`;
- разрешено явным перечислением: чтение, grep/find, `node --test`, `npm run lint`.

**Проверять за агентом руками:** после веера аудита — `git status`, `ls data/`,
`lsof -ti tcp:3000`. Отчёт агента о том, что он «всё убрал за собой», сам по себе
не доказательство.

**Второй вывод того же дня:** субагент может перепутать свою роль и вернуть
вместо отчёта фразу «жду завершения фоновых агентов». Лечится добивочным
сообщением «ты исполнитель, никого не ждёшь, выдай отчёт сейчас» — контекст у
него сохраняется, работа не пропадает. А ещё агент, породивший собственных
под-агентов, может отчитаться за непокрытое честным списком «не успел» — это
хороший исход, но означает, что нужен добивающий проход по остатку.

## 2026-07-27 — `npm audit fix` в этом проекте делает хуже, а счётчик audit врёт

**Что случилось:** закрывая уязвимости по итогам аудита, запустил штатный
`npm audit fix` (без `--force`, то есть «безопасный» режим). Результат: было
**10 уязвимостей, стало 17** (13 high). Он «чинит» eslint-цепочку, подтягивая
eslint 10.x, и тем самым заносит новые advisory в `@eslint/config-array`,
`@eslint/eslintrc`, `eslint-plugin-*` и `minimatch`. Откатил, обновил всё руками
по одному пакету.

**Правило:** в этом репозитории обновлять зависимости ТОЛЬКО точечно
(`npm install <pkg>@<version>`), сверяя `npm audit` до и после каждого шага.
`npm audit fix` не запускать — ни с `--force`, ни без.

**Второй, отдельный вывод — счётчик `npm audit` не метрика безопасности.**
Дважды за сессию число пугало без реальной причины:

1. После апгрейда next до 16.2.12 audit продолжал числить `next` уязвимым и
   предлагал «фикс» — даунгрейд до 9.3.3. Я на этом поспешно заявил, что
   апгрейд не помог. На самом деле каждый из девяти advisory помечен
   «уязвимы `<16.2.11`», а `via` содержал 0 собственных записей: next числился
   уязвимым ТРАНЗИТИВНО, через вендоренный внутри него postcss.
2. После апгрейда googleapis до 173 счётчик прыгнул 10 → 17. Все новые записи
   сводились к ОДНОМУ уже присутствовавшему `brace-expansion`: googleapis тянет
   `glob@10`/`rimraf@5`, те потребляют тот же уязвимый minimatch, и npm начал
   считать каждого потребителя отдельной строкой. Версии eslint-плагинов при
   этом побайтово те же (проверено `npm ls` до и после).

**Как читать audit правильно:** смотреть не `total`, а `via` каждой записи —
есть ли у пакета СВОИ advisory или он лишь потребитель чужого; и достижим ли
вектор в нашем коде (наш CSS, наши glob-паттерны, наши картинки — не
пользовательский ввод). Вывод о «стало хуже/лучше» делать по этому, а не по
числу. И не объявлять результат апгрейда, не заглянув в `via`.

## 2026-07-29 — `expunge()` без UIDPLUS удаляет ЧУЖИЕ письма с флагом `\Deleted`

Разбирая почту, я чистил ящики по схеме `UID COPY → Trash` → `UID STORE
+FLAGS \Deleted` → `expunge()`. Первые два шага адресные (по UID), а третий —
**нет**: `EXPUNGE` без аргументов сносит из папки ВСЕ письма, у которых стоит
`\Deleted`, независимо от того, кто их пометил. Почтовые клиенты и веб-морда
reg.ru помечают удалённое, но не всегда делают expunge сразу, так что в ящике
могут лежать чужие помеченные письма — и мой «точечный» проход добьёт их
безвозвратно, мимо Trash.

Проверил `CAPABILITY` обоих ящиков: **UIDPLUS сервер reg.ru НЕ поддерживает**,
то есть `UID EXPUNGE <set>` (RFC 4315), который снёс бы ровно мои UID,
недоступен. Обходного пути на уровне протокола нет.

**Правило:** перед `expunge()` всегда проверять, что помечено на удаление, и
сверять с собственным списком:

```python
typ, d = M.search(None, "DELETED")     # что уже помечено ДО моих действий
```

Если находится что-то, кроме моих целей — не делать `expunge()` вообще:
`COPY` в Trash уже выполнен, копия сохранена, а флаг `\Deleted` в INBOX
почтовый клиент юзера уберёт или добьёт сам. Пустой Trash хуже потерянного
письма.

**Проверять последствия — тоже правило.** Расхождение («в ящике должно было
остаться 2 письма, а он пуст») ловится сверкой Trash со своим списком: если
письма нет ни в INBOX, ни в Trash, значит его стёр expunge, а не перенос.
В этот раз обошлось (сервер вернул на expunge ровно мои 12 seq-номеров, а два
уцелевших письма исчезли позже и не от моих действий), но проверка должна быть
рефлексом, а не расследованием постфактум.

## 2026-07-31 — состояние окружения дрейфует: проверять перед каждым использованием, а не раз за сессию

Ночная вахта с заходами в кабинет Qwoted. В 01:50 проверил выход — Astana, KZ,
как того требует правило площадки (за location mismatch нас уже банили в июле).
Дальше работал в кабинете часами и больше не проверял: правил теги, вёл переписку
с журналисткой WSJ, перенастраивал «Tailor my alerts».

В 11:09 инбокс отдал **пустой каркас при HTTP 200**. Пошёл было чинить парсинг —
и только потом сверил гео: VPN отвалился молча, выход шёл через российский IP
(95.221.167.4, Лобня). Сколько запросов ушло с него до обнаружения — неизвестно.

**Правило:** проверка предусловия действительна ровно на один вызов, если само
предусловие может измениться без моего участия. VPN, авторизация, смонтированный
том, поднятый туннель — всё это дрейфует. В длинном цикле проверять В КАЖДОЙ
итерации, до первого обращения к защищённому ресурсу:

```bash
curl -s https://ipinfo.io/json | grep '"country"'   # не KZ → к Qwoted не ходить вообще
```

**Второе:** «пустой ответ вместо данных» диагностировать начинать с окружения
(гео, авторизация, сеть), а не с собственного кода. Пустая страница с 200 — это
почти всегда «нас не пустили», а не «сломался парсер».

**Третье:** заметив падение, сначала прекратить обращения (закрыть вкладки —
фоновые XHR стучатся сами) и предупредить юзера, и только потом разбираться.

## 2026-07-31 — «форма отправлена без ошибок» ≠ «дошло»: сабмит подтверждается только эффектом

Три недели считали, что заявка на бизнес-карточку expat.com «на модерации», — а
её у площадки никогда не было. Вскрытие 31.07: РФ-сеть (МегаФон, DPI) молча
рвёт multipart-POST к expat.com (`ERR_CONNECTION_RESET` даже на 184 КБ), при
этом GET и логин работают, а форма не показывает НИКАКОЙ ошибки — спиннер
исчезает, страница остаётся. Так 09.07 и родилось ложное «отправлено».

**Правило:** любой сабмит наружу (форма, заявка, питч) считается выполненным
только по положительному эффекту: ответ сервера с подтверждением/ID, письмо-
квитанция, объект в очереди кабинета. «Кликнул, ошибок не увидел» — не статус.
Проверять СРАЗУ, в той же сессии: сетевой лог (статус POST), затем вторичный
признак (письмо/кабинет/каталог).

**Механика обхода (пригодится снова):** DPI режет по размеру/типу тела, поэтому
браузерный ретрай бесполезен; решение — завернуть домен в KZ-VPN split-tunnel
(как qwoted.com) и отправить из браузера. Если UI после оборванного запроса
завис в «submitting» (кнопка мертва) — пересобрать FormData из перехваченного
слепка и отправить `fetch`-ом из контекста страницы: куки и origin те же,
заодно можно починить кривые скрытые поля (нам это дало верные координаты
вместо пустынного центроида «Dubai»).

**Смежное:** «аккаунт подтверждён» и «карточка опубликована» — разные состояния
разных объектов; сверять факт публикации в живом каталоге, а не по памяти.

## 2026-08-03 — данные для скрипта валидировать при СОЗДАНИИ, а не в день употребления

Августовский план автопостов был залит 31.07 без поля `poll_options` во всех пяти
опросах (в июльском плане поле было — потерялось при генерации нового месяца).
Обнаружилось это 02.08 в 06:00, когда `sendPollToChannel()` упал на
`post.poll_options.map(...)` с `TypeError: Cannot read properties of undefined`.
Деградация сработала штатно (try/catch, пост ушёл текстом через аппрув, алерт в
TG), но следующие четыре опроса — 09, 16, 23, 30 августа — легли бы точно так же,
по одному в неделю, каждый раз обнаруживаясь постфактум.

**Правило:** файл, который потребляет код по расписанию, проверяется против
кода-потребителя в момент создания файла, а не в момент первого запуска. Для
месячного плана это значит: сразу после заливки прогнать по всем элементам ту же
операцию, что сделает скрипт, — у `type: "poll"` обязаны быть `poll_options`
(1–12 штук, каждая ≤100 символов, лимит Telegram), у постов с картинкой —
непустой `imagePrompt`. Отложенный баг в кроне не «упадёт один раз», он
рассыпается порциями весь месяц.

**Смежное:** проверять полноту нужно СРАВНЕНИЕМ С ПРЕДЫДУЩИМ месяцем, а не
чтением схемы: поле пропало именно потому, что схема нигде не записана, а
образцом служил живой файл. `diff` ключей июль↔август нашёл бы это за секунду.

## 2026-08-03 — длинный текст согласовывать образцом, а не готовой вещью

Для Дзена я предложил три угла статьи, честно отметив, что вариант «прямо о
компании» читается в Дзене хуже остальных. Юзер выбрал именно его. Я написал 820
слов, прогнал через `humanizer`, оформил PR — текст не понравился, PR закрыт.

Ошибка не в выборе угла: выбор был юзера, и он имел на него право. Ошибка в том,
что я развернул слабый (по собственной оценке) вариант сразу до готового текста.
Выбирая угол, юзер отвечает на вопрос ЧТО, а тон, ритм, степень самопрезентации
и плотность фактуры остаются моим допущением — и разойтись может именно оно.

**Правило:** на тексте длиннее ~300 слов, а особенно когда я сам считаю выбранный
вариант рискованным, сначала показать образец — лид и один раздел, 150–200 слов
в предполагаемом тоне. Дальше разворачивать только после реакции. Образец стоит
десять минут, полный текст с вычиткой — час, и переписывать его целиком дороже,
чем написать заново.

**Смежное:** предупредить о риске и всё равно молча сделать по-своему полный
объём — это не «предупредил», это отчёт постфактум. Если риск реальный, дешёвая
проверка гипотезы должна быть частью предложения, а не следующей итерацией.

## 2026-08-03 — сверка данных без внешнего источника это не сверка

Августовская ежемесячная сверка доходности подняла цены по всем районам на
24–79% и снизила доходность в шести. Внутренне всё сошлось: шесть дублирующих
поверхностей были приведены друг к другу, сборка прошла, PR смержен. А через два
дня внешняя проверка перед рассылкой в СМИ нашла, что The Valley завышен на 37%
(1 850 против реальных 1 346), Emaar Beachfront занижен на 19% по цене и завышен
на два пункта по доходности, а верх диапазона JLT не подтверждается ничем.

Ошибка процессная: сверка свела наши цифры между собой вместо того, чтобы свести
их с рынком. Согласованность шести поверхностей доказывает только отсутствие
внутренних противоречий — она с равным успехом распространяет и верное значение,
и ошибочное.

**Правило:** в месячной сверке каждая цифра подтверждается ВНЕШНИМ источником, и
приоритет у данных по зарегистрированным сделкам (DLD, fäm, Cavendish Maxwell)
над индексами цен предложения (Bayut прямо пишет в дисклеймере, что публикует
asking prices, а не сделки). Если по району нашлась только одна слабая ссылка —
цифра остаётся, но в публичный питч не идёт.

**Второе, важнее:** прежде чем строить на своих данных публичное утверждение,
проверить, не опровергнуто ли оно уже. Наш тезис «дешёвый вход не даёт лучшую
доходность» был опровергнут статьёй Khaleej Times за четыре дня до рассылки — и
письмо ушло бы автору именно этой статьи. Проверять надо не только цифры, но и
вывод, который мы из них делаем.

**Третье:** таблица, где в одной колонке цена за фут квартир и вилл, сравнивает
разные величины. Разница «дешёвые районы дают меньше» объяснялась не ценой
входа, а типом актива: квартиры в Дубае дают 6,9%, виллы и таунхаусы 5,0%.

## 2026-08-04 — «такой фичи не существует» — проверять поиском, а не памятью

Сказал юзеру, что Google Search Console «не умеет добавлять соцсети» — а юзер
принёс пост из канала про platform properties (Instagram/TikTok/X/YouTube как
ресурсы GSC). Проверка показала: фича реальна, официальный анонс Search Central
07.07.2026 — на месяц новее моего среза знаний. Ошибка не в незнании (срез есть
срез), а в том, что отрицание было выдано уверенно и без проверки.

**Правило:** любое «такого нет / так нельзя» про внешний сервис (Google, соцсети,
платформы) — сначала быстрый свежий поиск с recency-фильтром, потом ответ.
Фичи выходят каждый месяц; отрицание устаревает быстрее, чем утверждение.
Особенно когда юзер приносит новость: «звучит неправдоподобно» ≠ «неправда» —
проверка заняла один поисковый запрос.

## 2026-08-05 — три вывода, сделанных быстрее, чем проверка

Все три в одной сессии, все три поправил юзер. Общий корень — вывод строился на
косвенном признаке, хотя прямая проверка стоила одного вызова.

**Первое: назвал настоящие отзывы фальшивыми.** В `orgJsonLd` лежали четыре
`Review` с именами James H., Priya S. и т.д. В карточке Google — восемь других
отзывов от других людей. Я объявил разметку выдуманной. Юзер: «у нас на сайте не
фальшивые отзывы, а тоже настоящие, только не из гугла». Вывод про разметку не
изменился (self-serving markup убирать всё равно надо), но обоснование было
обвинением, а не анализом.
**Правило:** не квалифицировать контент как поддельный по несовпадению с внешним
источником. Несовпадение = вопрос, а не приговор. Формулировать как «эти отзывы
не из Google — откуда они?» и спрашивать.

**Второе: «в статьях Дзена нет кликабельных ссылок».** Искал в HTML подстроку
`dzen.ru/away` (обёртка Дзена для внешних ссылок), не нашёл, объявил ссылки
плоским текстом и предложил юзеру их проставлять. Открыв редактор, увидел
`<a class="link" href="https://worldwise.pro/jlt">` — ссылки были, и хорошие, их
поставил юзер. Проблема была другой: у них не было UTM-меток.
**Правило:** отрицательный результат узкого поиска ≠ отсутствие явления. Если
гипотеза «ссылок нет» — искать `<a href`, а не конкретную обёртку конкретной
платформы. Прежде чем просить юзера о ручной работе, посмотреть на предмет
глазами, а не грепом.

**Третье, самое дорогое: сравнил платное с бесплатным.** В Дзен-студии увидел,
что статья набрала 7210 показов, а посты из RSS-фида — 0–2, и сделал
стратегический вывод: «Дзен раздаёт охват лонгридам, а RSS-фид как канал не
работает». Юзер: «эту статью мы рекламируем, потому у неё такие результаты. Ты
же сам настраивал рекламу, ты не помнишь?» Не помнил — это была прошлая сессия.
Кампанию на эту статью запустил я 01.08, всё записано в `~/ops-journal.md`.
**Правило:** контекст между сессиями не переносится, журнал переносится. Перед
любым выводом из статистики — `grep` по ops-journal на предмет собственных
прошлых действий с этим объектом. И отдельно: прежде чем объяснять разницу в
цифрах свойствами платформы, проверить, не объясняется ли она деньгами.
