...
**The rule:** перед браузерной задачей на площадке, где у юзера есть аккаунт, сначала подключаться к его ОСНОВНОМУ Chrome (живые сессии = ноль трения). Механизм выбрать при первом заходе и согласовать: CDP-attach к Chrome с debug-портом / Claude-in-Chrome / профиль в agent-browser. Изолированный Chromium — осознанный fallback (напр., чтобы не перехватывать фокус окна юзера — реальная гоча от 12.07), и тогда предупреждать, что потребуется логин.
