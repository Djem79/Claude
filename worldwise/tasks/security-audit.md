...
| H3 | ⚠️ Partial | Code now trusts only `cf-connecting-ip`/`x-real-ip` via shared `lib/ip.ts`; dropped client `x-forwarded-for`. **Still requires infra:** nginx must set `X-Real-IP $remote_addr`, and the origin must reject traffic that bypasses Cloudflare (firewall to Cloudflare IPs / Authenticated Origin Pulls). Not closed by code alone. |
