...
import { fmt, digitsOnly, isTgLead, tgHandle } from './lead-ui'
...
export default function LeadsClient({ initialLeads, isOwner = false }: { initialLeads: Lead[]; isOwner?: boolean }) {
  ...
  function exportCsv() {
    const cols: (keyof Lead)[] = ['createdAt', 'name', 'phone', 'email', 'budget', 'propertyType', 'area', 'source', 'utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content', 'gclid', 'fbclid', 'propertyTitle', 'status', 'notes']
    ...
    for (const l of filtered) {
      rows.push(cols.map(c => escape(l[c])).join(','))
    }
                ...
                <div key={status}>
                  ...
                  {columnLeads.map(l => (
                    <KanbanCard
                      key={l.id}
                      lead={l}
                      borderClass={CARD_BORDER[(l.status ?? 'new') as LeadStatus]}
                      onOpen={() => { setView('table'); setOpenId(l.id) }}
                    />
                  ))}
                ...
                <div>
                  {mobileLeads.map(l => (
                    <KanbanCard
                      key={l.id}
                      lead={l}
                      borderClass={CARD_BORDER[mobileColumn]}
                      onOpen={() => { setView('table'); setOpenId(l.id) }}
                    />
                  ))}
